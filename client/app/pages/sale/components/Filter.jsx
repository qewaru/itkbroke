...
export default function Filter(prices) {
  ...
  return (
    <header className="flex justify-center w-full h-[auto] pt-[45px] text-lg">
        ...
        <div className={`top-0 left-0 w-full h-screen bg-black/50 z-20 ${toggle ? 'fixed' : 'hidden'}`}></div>
        <aside className={`top-0 left-0 fixed flex flex-col justify-between z-20 p-6 bg-background w-[100%] h-full lg:w-[30%] semimd:w-[40%] md:w-[50%] transition duration-300 ease-in-out ${toggle ? 'translate-x-0' : 'translate-x-[-100%]'}`}>
